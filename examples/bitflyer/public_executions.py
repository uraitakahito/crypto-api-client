#!/usr/bin/env python3
"""Sample script to fetch public execution history from bitFlyer

Fetches market-wide execution history for a specified currency pair.
Uses Public API which does not require authentication.

.. code-block:: console

    # Fetch execution history for BTC/JPY pair
    uv run python examples/bitflyer/public_executions.py --pair BTC_JPY --zone Asia/Tokyo

    # Fetch execution history for ETH/JPY pair (debug mode)
    uv run python examples/bitflyer/public_executions.py --pair ETH_JPY --zone Asia/Tokyo --log-level DEBUG
"""
...
async def async_main(
    ...
    all_executions = await fetch_executions_with_paging(
        pair, total_count, batch_size, delay, zone
    )
...
async def fetch_executions_with_paging(
    ...
    """Fetch execution history with specified count using pagination

    :param product_code: Currency pair (product code)
    :param total_count: Total number of items to fetch
    :param batch_size: Number of items to fetch per request
    :param delay: Wait time between requests (seconds)
    :param zone: Timezone for display (data itself is kept in UTC)
    :return: List of fetched execution history
    """
    all_executions: list[PublicExecution] = []
    ...
    async with create_session(Exchange.BITFLYER) as session:
        while len(all_executions) < total_count:
            ...
            request = PublicExecutionsRequest(
                product_code=product_code,
                count=count,
                before=str(before_id) if before_id else None,
            )
            ...
            executions = await session.api.public_executions(request)
