#!/usr/bin/env python3
"""Sample script to fetch order book from GMO Coin

.. code-block:: console

    # Fetch order book for BTC/JPY
    uv run python examples/gmocoin/orderbook.py --pair BTC_JPY --price-band 100000 --rows 5

    # Fetch order book for ETH/JPY
    uv run python examples/gmocoin/orderbook.py --pair ETH_JPY --price-band 1000 --rows 7
"""
...
from crypto_api_client.gmocoin import OrderBook, OrderBookRequest
...
async def async_main(
    ...
    request = OrderBookRequest(symbol=pair)
...
def display_order_book(
    ...
    title = f"Order Book: {request.symbol} (Price Band: {price_band_str} | Spread: {order_book.spread:,.0f})"
