#!/usr/bin/env python3
"""Sample script to fetch order book (Depth) from bitbank

.. code-block:: console

    # Fetch order book for BTC/JPY
    uv run python examples/bitbank/depth.py --pair btc_jpy --price-band 100000 --rows 5

    # Fetch order book for XRP/JPY
    uv run python examples/bitbank/depth.py --pair xrp_jpy --price-band 0.1 --rows 5
"""
...
from crypto_api_client.bitbank import Depth, DepthRequest
...
async def async_main(
    ...
    request = DepthRequest(pair=pair)
...
def display_depth(
    ...
    title = f"Order Book: {request.pair} (Price Band: {price_band_str} | Spread: {depth.spread:,.0f})"
    display_order_book_table_with_bands(
        title, depth, rows, price_band, size_precision=6
    )
